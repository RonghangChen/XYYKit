...
// 读第一个帧 - (参考帧)
// 返回值: 0-出错; 1-正确
int ReadAMRFrameFirst(FILE* fpamr, unsigned char frameBuffer[], int frameBufferSize,int* stdFrameSize, unsigned char* stdFrameHeader)
{
	memset(frameBuffer, 0,  sizeof(unsigned char) * frameBufferSize);
...
// 返回值: 0-出错; 1-正确
int ReadAMRFrame(FILE* fpamr, unsigned char frameBuffer[],int frameBufferSize, int stdFrameSize, unsigned char stdFrameHeader)
{
//	int bytes = 0;
...
//	fpwave = fopen([docFilePath cStringUsingEncoding:NSASCIIStringEncoding], "wb");
	...
	ReadAMRFrameFirst(fpamr, amrFrame, MAX_AMR_FRAME_SIZE, &stdFrameSize, &stdFrameHeader);
	...
	while(1)
	{
		memset(amrFrame, 0, sizeof(amrFrame));
		memset(pcmFrame, 0, sizeof(pcmFrame));
		if (!ReadAMRFrame(fpamr, amrFrame, MAX_AMR_FRAME_SIZE, stdFrameSize, stdFrameHeader)) break;
